...
class BookingService {
   ...
   * Rooms are sorted and we sum consecutive distances
   */
  calculateTotalTravelTime(roomIds) {
    if (!roomIds || roomIds.length <= 1) return 0;

    // Sort rooms by floor, then by position (optimal traversal order)
    const sorted = [...roomIds].sort((a, b) => {
      const ra = this.parseRoomId(a);
      const rb = this.parseRoomId(b);
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      totalTime += this.calculateTravelTime(sorted[i], sorted[i + 1]);
    }
  ...
  /**
   * PHASE 2: Find best cross-floor allocation using sliding window
   * Minimizes total travel time across floors
   */
  findBestCrossFloorAllocation(count) {
    ...
    // Sort by floor, then by position
    available.sort((a, b) => {
    ...
    for (let i = 0; i <= available.length - count; i++) {
      ...
      const travelTime = this.calculateTotalTravelTime(window.map(r => r.id));
      ...
      if (travelTime < bestTravelTime) {
        ...
        bestAllocation = {
          rooms: window,
          travelTime,
          floors: [...new Set(window.map(r => r.floor))],
